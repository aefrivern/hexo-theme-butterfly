...
hexo.extend.helper.register('related_posts', function (currentPost, allPosts) {
  ...
  if (relatedPosts.length > 0) {
    ...
    result += `<div class="headline"><i class="iconfont icon-tuijian"></i><span>${headlineLang}</span></div>`;
    result += '<div class="relatedPosts-list">';

    for (let i = 0; i < Math.min(relatedPosts.length, limitNum); i++) {
      let { cover, title, path, cover_type, created, updated, postDesc } =
        relatedPosts[i];
      const { escape_html, url_for, date } = this;
      cover = cover || 'var(--default-bg-color)';
      title = escape_html(title);
      const className = postDesc
        ? 'pagination-related'
        : 'pagination-related no-desc';
      result += `<a class="${className}" href="${url_for(
        path
      )}" title="${title}">`;
      if (cover_type === 'img') {
      ...
      if (dateType === 'created') {
        result += `<div class="info text-center"><div class="info-1"><div class="info-item-1"><i class="far fa-calendar-alt fa-fw"></i> ${date(
          created,
          hexoConfig.date_format
        )}</div>`;
      } else {
        result += `<div class="info text-center"><div class="info-1"><div class="info-item-1"><i class="fas fa-history fa-fw"></i> ${date(
          updated,
          hexoConfig.date_format
        )}</div>`;
      }
